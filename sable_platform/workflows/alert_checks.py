...
def _check_workflow_failures(
    ...
    conditions = (f"WHERE status='failed' AND (created_at IS NULL"
                  f" OR {ts_column('created_at', _dialect)} > {_cutoff})")
...
def _check_discord_pulse_stale(conn: sqlite3.Connection, org_id: str) -> list[str]:
    ...
    if not row:
        ...
        # ("syntax error at or near :"), so the check silently errored and never
        # fired on the prod Postgres DB. Parsing the ISO timestamp in Python is
        # dialect-agnostic and removes the second SQL round trip entirely.
        from datetime import datetime, timezone
        ...
        try:
            ...
            log.warning("discord_pulse_stale: unparseable run_date %r for %s: %s", ts_raw, org_id, e)
            is_stale = True  # can't parse → treat as stale (fail safe)
