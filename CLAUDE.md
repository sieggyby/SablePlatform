...
| `sable_platform/db/orgs.py` | Org creation — `upsert_prospect_org()` (prospect orgs use `status='inactive'` + `config_json={org_type:'prospect', max_ai_usd_per_org_per_week:0.50}` — the cap under the EXACT key `cost.get_org_cost_cap` reads). Canonical SP-owned home for prospect-org creation (supersedes `SableKOL/wizard_orgs.py`). |
