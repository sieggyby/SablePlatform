...
### SP-SYNC-LAPTOP-TO-PROD: `sable-platform sync-from-local`
> **Shipped 2026-05-12.** New CLI command + `db/sync_from_local.py` module. Org-scoped incremental sync from a local SQLite source into the target DB, with per-table cursors in `platform_meta` and dialect-agnostic SA Core writes. 16 tables covered (entities, handles, diagnostic_runs by `cult_run_id`, sync_runs, artifacts, tag full-state replace, tag_history, interactions, decay, centrality, discord_pulse, metric_snapshots, playbook targets/outcomes, cost_events, merge_candidates). Audit-driven hardening: `--source-db` never auto-reads `SABLE_DATABASE_URL` (prevents accidental self-sync against prod), schema-version parity check aborts on mismatch, `entity_interactions` count is replaced not added (avoids `sync_interaction_edges` double-count bug), `entity_tags` does full state replace (mutable rows), alerts/workflow tables/diagnostic_deltas/audit_log are intentionally excluded. Stamps one summary audit_log row per non-dry-run sync (`source='sync-from-local'`). 15 tests in `tests/db/test_sync_from_local.py`.

### SP-API-MVP: Alert Triage HTTP API (private)
> **Shipped 2026-05-12.** Three v1 routes (`GET /v1/orgs/{org_id}/alerts`, `POST /v1/alerts/{alert_id}/acknowledge`, `POST /v1/alerts/{alert_id}/resolve`) over stdlib `ThreadingHTTPServer` (no new deps). Migration 044 adds `api_tokens` (SHA-256 hashed; raw secret returned once). Audit-driven hardening: `acknowledge_alert`/`resolve_alert` refactored to be idempotent and accept explicit `actor`/`source`/`detail_extra` (no env-based actor fallback for API writes); `get_alert` helper added so resource-ownership is checked before mutation; wrong-org alert access returns 404 (not 403) to avoid existence leaks; token verify uses `hmac.compare_digest` and burns time on unknown token_ids; default bind is `127.0.0.1` with explicit `--public` required for non-loopback; `api-serve` runs without `SABLE_OPERATOR_ID` (caller carries identity in token), but `api-token issue` requires it. Owner-only CLI: `sable-platform api-token issue/list/revoke` + `api-serve`. OpenAPI spec served at `/openapi.json`. 39 tests across `tests/api/test_tokens.py`, `test_alert_refactor.py`, `test_rate_limit.py`, `test_server.py`. Operator-facing summary: `docs/API_ALERT_TRIAGE_MVP.md`.
