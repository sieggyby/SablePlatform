> **SUPERSEDED / SHIPPED 2026-05-30 — retained for historical rationale; current state is in the code + SUITE_CAPABILITIES.md.** Only the un-shipped **PR-X4 (SableTracking `MEDIA_BACKEND=r2` cutover)** remains open; everything else (Worker, signer, clip-suggest endpoint, `/ops/clip-assist`) is live.
